// This solution is way overcomplicated and completely unrelated to techniques that will show up in Programmeringsolympiaden
// If you're still curious, this solution works by formulating the problem as an integer linear optimization problem
// We don't have an ILP solver, only LP, so we use the LP solver as a black box with branch and bound to solve the ILP version
#include <bits/stdc++.h>
using namespace std;
...
signed main()
{
	...
	rep(i, n)
	{
		int c;
		cin >> c;
		whomst[c].push_back(i);
	}

	repe(who, whomst)
	{
		vd row(n);
		repe(w, who.second) row[w] = 1;
		A.push_back(row);
		b.push_back(k);
	}
	integer_constraint.resize(n, 1);
