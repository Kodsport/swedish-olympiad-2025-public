...
public class karleksbrev {
    ...
    static final UnsignedLong MOD = new UnsignedLong(BigInteger.valueOf(1125899906842679L));

    // Hashf method to compute the hash for a given string
    static UnsignedLong Hashf(String x) {
        UnsignedLong ret = new UnsignedLong(0L);
        for (char v : x.toCharArray()) {
            ret = ret.add(new UnsignedLong(v == '1' ? 1L : 0L));
            ret = ret.multiply(new UnsignedLong(1087L)).mod(MOD);
        }
        return ret;
    }
    ...
    static String Hashstring(String x) {
        ...
        UnsignedLong h = Hashf(x);

        for (int j = 0; j < k; j++) {
            // Check the bit at position j using the bitwise AND
            if (h.toBigInteger().testBit(j)) {
                ret[j] = '1';
            }
        }
